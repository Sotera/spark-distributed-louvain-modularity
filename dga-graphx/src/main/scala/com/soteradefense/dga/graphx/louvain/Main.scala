...
case class Config(
                   ...
                   output: String = "",
                   master: String = "local",
                   appName: String = "Distributed Community Detection",
                   jars: String = "",
                   sparkHome: String = "",
                   parallelism: Int = -1,
...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    var edgeRDD = sc.textFile(edgeFile).map(row => {
      ...
      tokens.length match {
        case 2 => {
          new Edge(inputHashFunc(tokens(0)), inputHashFunc(tokens(1)), 1.0)
        }
        case 3 => {
          new Edge(inputHashFunc(tokens(0)), inputHashFunc(tokens(1)), tokens(2).toDouble)
        }
        case _ => {
